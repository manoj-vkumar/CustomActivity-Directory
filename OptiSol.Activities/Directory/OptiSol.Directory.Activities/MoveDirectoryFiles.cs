...
namespace OptiSol.Directory {
    public class MoveDirectoryFiles : CodeActivity {
        //File(s) will be cut from this directory path.
        [Category("Input")]
        [RequiredArgument]
        ...
        private void MoveFilesToDirectory(CodeActivityContext context) {
            try {
                ...
                foreach (string file in HelpperClass.GetDirectoryFiles(DirectoryPath: DirectoryPathFrom.Get(context), FileExtension: FileExtension.Get(context), CreatedBy: CreatedBy.Get(context), DayCountFrom: DayCountFrom.Get(context), DayCountTo: DayCountTo.Get(context))) {
                    ...
                    try {
                        ...
                        //Move a files one by one from one directory into another
                        System.IO.File.Move(file, pathTo);
                    } catch (Exception ex) {
                        throw ex;
                    }
                }
